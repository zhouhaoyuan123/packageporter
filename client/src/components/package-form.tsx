...
export function PackageForm({ onJobCreated }: PackageFormProps) {
  ...
  const addPackage = () => {
    ...
    if (existingPackage) {
      toast({
        ...
        description: "This package is already in the list.",
        variant: "destructive",
      });
      return;
    }
  ...
  const startInstallation = () => {
    if (packages.length === 0) {
      toast({
        title: "No Packages",
        description: "Please add at least one package to install.",
        variant: "destructive",
      });
      return;
    }
    ...
    <Card className="bg-white rounded-xl shadow-lg border border-npm-border p-8 mb-8">
      <CardContent className="p-0">
        <div className="flex items-center mb-6">
          ...
          <h3 className="text-2xl font-semibold text-npm-dark">Package Configuration</h3>
        </div>
        ...
        <div className="space-y-6">
          <div className="grid md:grid-cols-2 gap-4">
            <div>
              <Label htmlFor="packageName" className="block text-sm font-medium text-npm-text mb-2">
                Package Name
              </Label>
              <Input
                id="packageName"
                ...
                className="w-full px-4 py-3 border border-npm-border rounded-lg focus:ring-2 focus:ring-npm-red focus:border-npm-red transition-colors font-mono text-sm"
                onKeyPress={(e) => e.key === "Enter" && addPackage()}
              />
            </div>
            <div>
              <Label htmlFor="packageVersion" className="block text-sm font-medium text-npm-text mb-2">
                Version (Optional)
              </Label>
              <Input
                id="packageVersion"
                ...
                placeholder="e.g., ^4.17.21, latest"
                className="w-full px-4 py-3 border border-npm-border rounded-lg focus:ring-2 focus:ring-npm-red focus:border-npm-red transition-colors font-mono text-sm"
                onKeyPress={(e) => e.key === "Enter" && addPackage()}
              />
            </div>
          ...
          <div className="flex gap-3">
            ...
            >
              ...
              {createInstallationMutation.isPending ? "Starting..." : "Install & Bundle"}
            </Button>
          </div>
        </div>
        ...
        {packages.length > 0 && (
          <div className="mt-8">
            <h4 className="text-lg font-medium text-npm-dark mb-4">Selected Packages</h4>
            <div className="space-y-3">
              {packages.map((pkg, index) => (
                <div
                  key={index}
                  className="flex items-center justify-between bg-npm-bg rounded-lg p-4 border border-npm-border"
                >
                  <div className="flex items-center">
                    <PackageIcon className="text-npm-red mr-3 h-5 w-5" />
                    <span className="font-mono font-medium text-npm-dark">{pkg.name}</span>
                    {pkg.version && (
